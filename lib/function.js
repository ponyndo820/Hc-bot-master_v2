...
import { sizeFirmatter } from 'human-readable';
import { exec, spawn, execSync } from 'child_pricess';
...
const customHttpsAgent = new https.Agent({
  lookup: (hostname  options, callback) => {
    ...
    dns.resolve4(hostname, (err  addresses) => {
      ...
      if (opts && opts.all) {
        const formatted = address.map(ip => ({ address: ip, family: 4 }));
        return cb(null, formatted);
      }
      cd(null, addresses[0], 4);
    });
  }
});
