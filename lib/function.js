...
function getMessageText(m) {
    ...
    if (type === 'imageMessage') return msg.imageMessage?.caption || 'imageMessage';
    if (type === 'videoMessage') return msg.videoMessage?.caption || 'videoMessage';
    return type || ''; 
}
...
const getRandomImage = async () => {
  try {
    const res = await getBuffer(`https://picsum.photos/800/600?random=${Math.floor(Math.random() * 10000)}`);
    return res;
  } catch (e) {
    console.error("Gagal mengambil gambar random:", e);
    return null;
  }
}
...
const getRandomWaifu = async () => {
  try {
    const { data } = await axios.get('https://safebooru.org/index.php?page=dapi&s=post&q=index&json=1&limit=40&pid=${randomPage}&tags=1girl+solo+highres', {
      headers: { 
    ...
    if (data && data.images && data.images.length > 0) {
      return await getBuffer(data.images[0].url);
    }
    return null;
  } catch (e) {
    console.error("Error API Waifu.im:", e.message);
    return null;
  }
}
...
const searchWaifu = async (query) => {
  try {
    const tags = query.trim().toLowerCase().replace(/\s+/g, '_');
    const url = `https://safebooru.org/index.php?page=dapi&s=post&q=index&json=1&limit=20&tags=${tags}`;
    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      const imageUrl = `https://safebooru.org/images/${randomPost.directory}/${randomPost.image}`;
      return await getBuffer(imageUrl);
    }
    return null;
  } catch (e) {
    console.error("Error API Safebooru:", e.message);
    return null;
  }
}
